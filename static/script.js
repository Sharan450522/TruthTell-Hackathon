...
// UI functions
const UI = {
    appendVideoFragment(fragmentBase64) {
        const img = document.createElement('img');
        img.src = 'data:image/jpeg;base64,' + fragmentBase64;
        img.className = 'w-32 h-20 object-cover rounded shadow';
        document.getElementById('fragmentContainer').appendChild(img);
      },
      clearVideoFragments() {
...
socket.on('video_fragment', (data) => {
    if (data && data.fragment) {
      UI.appendVideoFragment(data.fragment);
    }
  });
